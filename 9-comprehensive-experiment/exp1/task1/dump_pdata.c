...
void CopyProcDataSection(const char *path)
{
    ...
    uPtr = task->mm->mmap->vm_start;
    dataLen = task->mm->mmap->vm_end - task->mm->mmap->vm_start;
    len = dataLen;
    ...
    while (len > 0)
    {
        ...
        long get_user_pages_remote(
            struct task_struct *tsk,//需要钉住的进程结构体 
            struct mm_struct *mm,//目标进程的 mm 结构体 
            unsigned long start, //用户空间起始地址
            unsigned long nr_pages,//从 start 开始的页面数量 
        ...
        */
        err = get_user_pages_remote(task, task->mm, uPtr, 1, FOLL_FORCE, &page, NULL, NULL);  
        if (err < 0)
        {
